//https://youtu.be/Sc7STdHcPC4?si=8uMy9HeaWXqQJa_4
...
class Pantalla {
  constructor() {
    ...
    this.boton = new Boton(width / 2, height / 2 + 50, 100, 50);
    this.botonReinicio = new Boton(width / 2, height / 2 + 150, 100, 50);
    this.estado = "inicio";
    this.sonidoReproduciendo = false; // Nueva variable para controlar el estado del sonido
  }
  ...
  dibujar() {
    if (this.estado === "inicio") {
      ...
      text("Desplazate hacia los costados con las letras A y D \n Si no logras agarrar los 10 insectos perderas vidas ", width / 2, 200);
       
      this.boton.dibujar("Iniciar");
      if (this.boton.presionar()) {
        this.estado = "juego";
        if (sonido.isLoaded() && !this.sonidoReproduciendo) { // Solo reproducir si no está reproduciendo
          sonido.loop();
          this.sonidoReproduciendo = true; // Cambia el estado a reproduciendo
        }
      }
    }
